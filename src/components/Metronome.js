...
class Metronome extends React.Component {
  ...
  constructor(props) {
    ...
    this.ticksPerBeat = this.props.beatsPerMeasure * this.props.subdivision
  ...
  runScheduler = () => {
    while (
      ...
      //adding new update
      this.ticksPerBeat = this.state.beatsPerMeasure * this.state.subdivision
        /* this.state.beatsPerMeasure % 3 === 0 || this.state.subdivision % 3 === 0
        ? TICKS_PER_BEAT_TERNARY
        : TICKS_PER_BEAT_BINARY */
  ...
  onTSChange = beatsPerMeasure => {
    this.timerWorker.postMessage({
      action: ACTION_UPDATE,
  
    })
    this.setState({
      beatsPerMeasure:parseInt(beatsPerMeasure,0),
    })

  }

  onSubDivChange = subdivision => {
    this.timerWorker.postMessage({
      action: ACTION_UPDATE,
    })
    //let subdivision = event.target.value
    this.setState({
      subdivision:parseInt(subdivision,10),
    })
  }
  ...
  render() {
    return this.props.render({
      ...
      onTSChange: this.onTSChange,
      onSubDivChange: this.onSubDivChange,
      onPlay: this.onPlay,
    })
  }
}
